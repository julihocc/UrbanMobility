---
description: "Use when editing code or documentation in the UrbanMobility repository. Prefer preserving the simulation architecture, reproducibility, and analysis workflow."
...
- Prefer deterministic experiment setup for final or reporting runs: set or thread a seed in entry points and example script executions when randomness is used.
- Prefer adding new simulation parameters in one place and keeping names consistent across main.py, Reporting.py, and example script usage.
...
- Prefer script-first examples under `examples/` as plain `.py` files for focused workflows (quickstart, sweeps, visualization, serialization, GUI adapter usage).
- Prefer examples that run deterministically from top to bottom as plain Python scripts without hidden state assumptions.
- Prefer keeping examples import-safe by using `if __name__ == "__main__":` guards when practical.
